...
import com.example.pbanking.consent.dto.response.ProductConsentResponse;
...
public class ConsentService {
    ...
    private final ConsentAccessService consentCacheService;
    ...
    public AccountConsentResponse getReadConsent(String bankId, String clientId) {
        ...
        consentCacheService.saveAccountConsent(response, bankId, clientId, user);
        return response;
    }
    ...
    public String getConsentForBank(String bankId, ConsentType consentType, Map<String, String> kwargs) {
        ...
        return switch (consentType) {
            case READ -> consentCacheService.resolveAccountConsent(bankId, user);
    ...
    private void saveProductConsent(ProductConsentResponse response, ProductConsentApiRequest requestBody,
            String clientId) {
    ...
    // private String resolveProductReadConsent(String bankId, User user,
    // Map<String, String> kwargs) {
    // BankEntity bank = bankService.getBankFromId(bankId);
    // ProductConsent consent =
    // productConsentRepository.findProductReadConsent(user, bank,
    // kwargs.get("productType"))
    // .orElseThrow(() -> new NotFoundException("No read product consent for user: "
    // + user.getUsername()
    ...
    // private String resolveProductOpenConsent(String bankId, User user,
    // Map<String, String> kwargs) {
    // BankEntity bank = bankService.getBankFromId(bankId);
    // ProductConsent consent =
    // productConsentRepository.findProductOpenConsent(user, bank,
    // kwargs.get("productType"))
    // .orElseThrow(() -> new NotFoundException("No read product consent for user: "
    // + user.getUsername()
    // + ", bank: " + bankId + " and product type: " + kwargs.get("productType")));
    ...
    // private String resolveProductCloseConsent(String bankId, User user,
    // Map<String, String> kwargs) {
    // BankEntity bank = bankService.getBankFromId(bankId);
    // ProductConsent consent =
    // productConsentRepository.findProductCloseConsent(user, bank,
    // kwargs.get("productType"))
    // .orElseThrow(() -> new NotFoundException("No read product consent for user: "
    // + user.getUsername()
    // + ", bank: " + bankId + " and product type: " + kwargs.get("productType")));
    ...
    // private String resolveProductConsentValue(ProductConsent consent, String
    // bankId) {
    // if (consent.getStatus() == ConsentStatus.pending) {
    // String pendingId = encryptionService.decrypt(consent.getConsent());
    ...
    // } catch (RuntimeException ex) {
    // throw new NotFoundException(
    // "Stored consent is invalid or expired for bank: " + bankId + ". Please
    // request a new consent.");
    // }
    // }
